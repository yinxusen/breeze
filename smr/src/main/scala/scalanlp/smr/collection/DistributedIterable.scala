...
trait DistributedIterableLike[+T,+Repr<:DistributedIterable[T],+Local<:Iterable[T] with IterableLike[T,Local]] extends HackGenTraversableLike[T,Repr] {
  ...
  def local: Local = {
    val b = localBuilder;
    implicit val read = distributedBuilderFactory.serializeB;
    implicit def readIterT: DataSerialization.ReadWritable[Iterable[T]] = DataSerialization.iterableReadWritable(readableT);
  ...
  def map[B,That](f: T=>B)(implicit cbf: CanBuildFrom[Repr,B,That]) = cbf ifDistributed { dbf =>
    ...
    implicit def readIterT: DataSerialization.ReadWritable[Iterable[T]] = DataSerialization.iterableReadWritable(readableT);
    implicit val bser = dbf.serializeB;
...
//  def isEmpty = !nonEmpty;
  ...
  protected def exec[R:DataSerialization.Writable,S,T:DataSerialization.Readable](shard: URI, task: Task[T,R,S]) = {
...
object DistributedIterable {
  implicit def builder[T,U:DataSerialization.ReadWritable]: CanBuildDistributedFrom[DistributedIterable[T],U,DistributedIterable[U]] = new CanBuildDistributedFrom[DistributedIterable[T],U,DistributedIterable[U]] {
    ...
    def apply(t: DistributedIterable[T]): DistributedBuilder[U, DistributedIterable[U]]  = {
      new DistributedBuilder[U,DistributedIterable[U]]  {
        ...
        def resultFromSummaries(summaries: IndexedSeq[(Iterable[URI], LocalSummary)]) = {
    ...
    def serializeB = implicitly[DataSerialization.ReadWritable[U]];
  }

  private def localSerBuilder[Elem] = new LocalBuilder[Elem,Iterable[Elem],Int] {
    def summary() = innerBuilder.size;
...
private[smr] class SimpleDistributedIterable[+T:DataSerialization.ReadWritable](val shards: IndexedSeq[URI], sizes: IndexedSeq[Int], val distributor: Distributor with Storage) extends DistributedIterable[T] {
